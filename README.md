...
- **Semantic commit subjects** *(on by default, with git-autosnapshot)* — instead of a generic timestamp, a *meaningful* memory write (an insight promoted/demoted, a belief superseded, a note resolved, a dedup-merge) stamps a **why** subject on its snapshot commit — so the memory history reads like a changelog, not a wall of `auto-snapshot`. Zero new per-tick cost (the reason is already computed), zero LLM, fail-open. See [git-versioned memory](#git-versioned-memory-git-autosnapshot).
- **Config crash-loop protection** *(opt-in)* — `openclaw.json` is the one file a bad write can crash-loop the whole gateway on. The **config-guard** watchdog snapshots the last-good config and instantly reverts any syntax-broken write, so the gateway can't be taken down by a malformed edit. Independent systemd `.path` unit; degrades gracefully without `jq`/systemd. See [features/config-guard](features/config-guard/README.md).
...
| **Code-structure graph** | A **ZERO-LLM** map of your own codebase — *what calls this function, what does it call, where is this class defined, what's the path from A to B* — answered from a tree-sitter AST index (`file:line`) instead of grepping whole files into context (~20× fewer tokens on big repos). Project-tagged with cross-repo `depends_on`, refreshed daily like docs, and fused into recall so "what X does **and** why it exists" comes back in one answer. Searchable via `code_query` |
