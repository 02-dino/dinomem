...
| `tools.deny` | remove `sessions_spawn` if present | dinomem's Project Advancer and memory pipeline rely on `sessions_spawn` to delegate bounded sub-tasks. If `sessions_spawn` is in the denylist, project execution silently falls back to single-turn inline work and overflows context. Remove it from deny; use `tools.allow` per-agent if you need scoped access instead. |
