...
| **Long-document RAG** | Contracts, books, legal text — **plus images, scanned pages (auto-OCR'd via the agent's own vision model, no GPU), audio/video files, ZIPs, and YouTube/X/TikTok/Vimeo links** — all ingest automatically. Video links run a four-rung ladder: hosted caption API (opt-in `SUPADATA_API_KEY`, beats flagged/datacenter IPs) → free yt-dlp captions → download + transcribe → graceful skip. Stored separately, never pollutes memory, searchable via `docs_search` |
