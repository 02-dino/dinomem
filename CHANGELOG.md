...
### Fixed
- **Bare-relative filesystem paths stored in memory silently break on later reads.** The memory extractor (`extract_memory.py`) copies whatever path an agent typed during a session verbatim into `memory/YYYY-MM-DD_*.md`. When an agent wrote a workspace-relative path (e.g. `workspace-foo/kb/x.json`) instead of an absolute one, a *later* session reading that note from a different working directory resolves it wrong — on installs where the working directory equals the workspace name, the path silently doubles (`workspace-foo/workspace-foo/kb/...`) and the read fails with ENOENT. The extractor prompt was **silent** on path format, so nothing steered the LLM toward absolute paths at authorship time. Fixed: added an explicit `FILE PATHS MUST BE ABSOLUTE` rule to the extractor prompt — filesystem paths must be stored as full absolute paths (starting with `/`), never workspace-relative, with an instruction to expand a relative path to absolute before storing. Fixing this at authorship (the prompt that *generates* the note) keeps it portable and workspace-agnostic — no hardcoded workspace root, no false-positive risk from a post-hoc path-rewriting heuristic. (Ported from dinomem-neuron 1.7.5.)
- **Memory-extraction false "FAILED" during large/first-time backlog (misleading orchestrator status).** `extract_memory.py` processes archived sessions through an LLM in small batches; the orchestrator (`auto_session_reset.py`) wraps it in a hard `subprocess.run(timeout=300)`. On a large backlog (fresh install with lots of pre-existing history, a cron/hook outage that let archives pile up, or just a busy `/new`-heavy user), one 300s tick can't clear everything, so the subprocess gets killed mid-batch and every 15-min run logged `"Memory extraction: ⚠️ FAILED"` — even though it wasn't broken: `extract_memory.py` dedups via `.processed_archives.json` and advances the backlog a bit further on every tick (self-healing over time). This is a **misleading false alarm**, not a real failure, and it teaches users to ignore the log (alarm fatigue) or panic over nothing. Fixed: `extract_memory.py` now writes a small status file (`logs/.extract_memory_status.json`) **after every batch**, not just at the end — so it survives the orchestrator's SIGKILL-on-timeout — recording whether the run is still draining a backlog vs. actually failed (0 archives cleared). `auto_session_reset.py` reads it and reports `"⏳ IN PROGRESS (N archive(s) remaining, self-healing — not a real failure)"` instead of `"⚠️ FAILED"` when it's just backlog, reserving `FAILED` for when nothing was actually cleared. Backward-compatible: if the status file is missing (older `extract_memory.py`, or the subprocess died before writing anything), it falls back to the old blanket `FAILED` wording — never silently hides a real failure.
