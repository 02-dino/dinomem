...
## dinomem
  ...
  subagent_build_discipline:
    scope: task delegated via sessions_spawn that WRITES/EDITS code (gate|hook|skill|tool|lib|installer|config)
    spawn_side (orchestrator/parent):
      - build/self-mod task -> spawn with lightContext:false (full AGENTS.md, rule below not truncated)
      - read/summarize/research/data task -> lightContext:true (default; cheap)
      - name required skills IN the brief text explicitly (don't rely on subagent attention)
    worker_side (subagent):
      - task writes/edits code -> MUST read skill build-quality BEFORE first line
      - touches openclaw.json/config -> ALSO read openclaw-config-edit (never raw-edit; config patch/set only)
      - touches files (mv/rm/overwrite) -> ALSO read safe-file-ops
      - no commit/push/release unless brief explicitly grants it
DINOMEM_AGENTS_BODY
)
BLOCK="$BEGIN
$DINOMEM_BODY
...
echo "  5. Verify install:    bash $SKILL_DIR/scripts/verify_install.sh --workspace $WS"
echo "                        (checklist: crons, procedures, config — should end 'VERIFY_INSTALL: OK')"
echo ""
echo "  \033[1mℹ️  Where dinomem's crons live (READ THIS if a cron looks 'missing'):\033[0m"
echo "    dinomem's recurring jobs are registered into the SYSTEM CRONTAB — list them with:"
echo "        crontab -l | grep dinomem"
echo "    They do NOT appear in 'openclaw cron list' (that reads the separate Gateway SQLite"
echo "    store, which holds only the zero-LLM 'Note Cron Gate' dispatcher). Two different"
echo "    schedulers — checking the wrong one makes a healthy install look incomplete."
echo ""
echo ""
