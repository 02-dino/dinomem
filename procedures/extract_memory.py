...
- FILE PATHS MUST BE ABSOLUTE: when storing any filesystem path, write it as a full absolute path (starting with /), NEVER workspace-relative (e.g. 'workspace-foo/kb/x.json'). A relative path silently breaks when a later session reads it from a different working directory. If the session showed a relative path, expand it to absolute before storing.
