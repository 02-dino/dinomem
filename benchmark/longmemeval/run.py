...
# Ordered so the $ estimate is roughly right for common tiers. NOTE: matching is
# LONGEST-KEY-FIRST (see _price) so 'gpt-4o-mini' is not swallowed by 'gpt-4o'.
PRICE_HINT = {
    # budget / cheap tier (where INGEST should run)
    "gpt-4o-mini": 0.0004, "gpt-4o-mini-2024-07-18": 0.0004,
    "haiku": 0.0006, "flash": 0.0004, "mini": 0.0004,
    "gemini-3-flash": 0.0004, "gemini-flash": 0.0004,
    # mid tier
    "gpt-4o": 0.0075, "gpt-4o-2024-08-06": 0.0075,
    "sonnet": 0.006, "gemini-3-pro": 0.006, "gemini-pro": 0.006,
    # frontier
    "opus": 0.03, "gpt-5": 0.02,
    "default": 0.002,
}
...
def _price(model: str) -> float:
    # LONGEST matching key wins, so 'gpt-4o-mini' beats 'gpt-4o' and 'gemini-3-flash'
    # beats 'flash'. Prevents a cheap model being priced at a pricier substring tier.
    m = (model or "").lower()
    best_k, best_v = None, None
    for k, v in PRICE_HINT.items():
        if k == "default":
            continue
        if k in m and (best_k is None or len(k) > len(best_k)):
            best_k, best_v = k, v
    return best_v if best_v is not None else PRICE_HINT["default"]

# Ingest runs on dinomem's CHEAP (non-reasoning) tier, NOT the answerer. Resolve
# the SAME way extract_memory does so the $ footnote reflects the real split
# instead of worst-casing ingest at the answerer's price. Precedence (reused from
# the shared _cheap_model helper, single source of truth):
#   DINOMEM_CHEAP_MODEL env -> agents.defaults.compaction.model anchor
#   -> DINOMEM_NO_REASONING_MODEL legacy -> "" (rides OpenClaw default = PRICEY).
def _resolve_cheap_model(cli_override: str = "") -> tuple[str, str]:
    """Return (model, source). Empty model => nothing configured; ingest will ride
    the OpenClaw agent default, which is typically the PRIMARY (expensive) model.
    source is a short human tag for the estimate printout + warning."""
    if (cli_override or "").strip():
        return cli_override.strip(), "--cheap-model"
    if os.environ.get("DINOMEM_CHEAP_MODEL", "").strip():
        return os.environ["DINOMEM_CHEAP_MODEL"].strip(), "DINOMEM_CHEAP_MODEL env"
    # reuse dinomem's own resolver (compaction anchor + legacy env) if importable
    try:
        import importlib.util as _ilu
        src = (os.environ.get("DINOMEM_WORKSPACE", "") or "").strip()
        for cand in (
            src and str(Path(src) / "procedures" / "_cheap_model.py"),
            str(Path(__file__).resolve().parents[2] / "procedures" / "_cheap_model.py"),
        ):
            if cand and Path(cand).exists():
                spec = _ilu.spec_from_file_location("_cheap_model", cand)
                mod = _ilu.module_from_spec(spec); spec.loader.exec_module(mod)
                m = (mod.cheap_model() or "").strip()
                if m:
                    return m, "compaction.model anchor"
                break
    except Exception:
        pass
    if os.environ.get("DINOMEM_NO_REASONING_MODEL", "").strip():
        return os.environ["DINOMEM_NO_REASONING_MODEL"].strip(), "DINOMEM_NO_REASONING_MODEL env"
    return "", "UNSET"


def cost_estimate(n: int, answer_model: str, judge_model: str,
                  arm: str = "base", dataset: str = "longmemeval",
    ...
    # INGEST runs on dinomem's CHEAP tier (extract_memory NO_REASONING_MODEL), NOT
    # the answerer. Resolve it the same way the pipeline does, and price ingest at
    # THAT model. If nothing is configured (UNSET), ingest rides the OpenClaw agent
    # default -> typically the PRIMARY (expensive) model -> we price it at the
    # answerer tier AND raise a cost warning, because that is the unnecessarily
    # costly path this whole feature exists to flag.
    resolved_cheap, cheap_source = _resolve_cheap_model(cheap_model)
    cheap_unset = (cheap_source == "UNSET")
    # UNSET => price ingest at answerer (worst case + warn); else at the cheap model.
    ingest_price_model = answer_model if cheap_unset else resolved_cheap
    ingest_cost = ingest_tokens / 1000.0 * _price(ingest_price_model)
    ...
    cost_warning = None
    if ingest_tokens and cheap_unset:
        cost_warning = (
            "DINOMEM_CHEAP_MODEL (and compaction.model anchor) is UNSET -> ingest "
            "(the 93%+ dominant bucket) will ride your OpenClaw agent DEFAULT model, "
            "usually your PRIMARY/reasoning tier. That is UNNECESSARILY COSTLY: set "
            "DINOMEM_CHEAP_MODEL (or agents.defaults.compaction.model) to a budget "
            "model (flash/mini/haiku) BEFORE a full run to cut the bill ~10-50x."
        )
    return {
        "n_questions": n,
        "arm": arm,
        ...
        # --- cheap-tier ingest resolution (the real cost driver) ---
        "ingest_model": (resolved_cheap or "OpenClaw-default (UNSET!)"),
        "ingest_model_source": cheap_source,
        "ingest_priced_at": ingest_price_model or "gateway-default",
        "cheap_unset": cheap_unset,
...
def estimate_all(answer_model: str, judge_model: str, cheap_model: str = "") -> dict:
    ...
    # worked $ using the RECOMMENDED pair (sonnet-4.6 answerer + gpt-4o judge) for
    # answer+judge, and the RESOLVED CHEAP model for INGEST (that is where ingest
    # actually runs — NOT the answerer). If cheap is UNSET, ingest rides the pricey
    # default: price it at the answerer tier and surface a warning.
    resolved_cheap, cheap_source = _resolve_cheap_model(cheap_model)
    cheap_unset = (cheap_source == "UNSET")
    a = RECOMMENDED_ANSWER_PRICE_PER_MTOK / 1e6
    j = RECOMMENDED_JUDGE_PRICE_PER_MTOK / 1e6
    # _price returns $/1K tokens -> convert to $/token (/1000) to match a & j units.
    ing = (a if cheap_unset else _price(resolved_cheap) / 1000.0)
    recommended_usd = tot_ingest * ing + tot_answer * a + tot_judge * j
    all_cost_warning = None
    if cheap_unset:
        all_cost_warning = (
            "DINOMEM_CHEAP_MODEL / compaction.model anchor UNSET -> the ~94% INGEST "
            "bucket rides your pricey DEFAULT model. Set a budget cheap model before "
            "a full run to cut this bill drastically.")
...
def print_cost(est: dict):
    ...
    if _ing:
        ...
        print(f"    ingest model:  {est.get('ingest_model','?')}  "
              f"(from {est.get('ingest_model_source','?')}; priced at {est.get('ingest_priced_at','?')})",
              file=sys.stderr)
    else:
...
def main():
    ...
    ap.add_argument("--cheap-model", default=os.environ.get("DINOMEM_CHEAP_MODEL", ""),
                    help="model the INGEST (dominant) bucket runs on, for the $ estimate. "
                    "Defaults to DINOMEM_CHEAP_MODEL / compaction.model anchor (same as "
                    "the real pipeline). If nothing resolves, the estimate WARNS that "
                    "ingest will ride your pricey default model.")
    ...
    # ESTIMATE-GATE: a --full (paid) run must SHOW the cost estimate before it can
    # spend. On --full without --yes we print the full-run estimate (incl. the
    # cheap-model cost warning) and STOP, so nobody launches a paid run blind.
    # --yes then means "I have seen the estimate above, proceed."
    if args.full and not args.yes:
        _log("--full is a PAID run. Showing the cost estimate FIRST (no spend yet):")
        _gate_est = cost_estimate(
            500, args.answer_model, args.judge_model,
            arm=args.arm, dataset=getattr(args, "dataset", "longmemeval"),
            cheap_model=getattr(args, "cheap_model", ""))
        print_cost(_gate_est)
        _fail("Estimate shown above. This is a paid citation-grade run — re-run with "
              "--yes to confirm you accept the cost."
              + ("  ⚠️  FIX THE COST WARNING FIRST (set DINOMEM_CHEAP_MODEL)."
                 if _gate_est.get("cost_warning") else ""))
    if not args.source:
