...
# ── SAFETY FLOOR (shared with route.py's forbidden_write_targets) ──────────────
# route.py DOCUMENTS these as never-write targets; here we ENFORCE it at the leaf
# so the rule is a mechanism, not a plea. MEMORY.md is already excluded from
# ALLOWED_FILES (blocked wholesale). USER.md is allowed, BUT its marker-bounded
# managed block is COMPILED by compile_user.py and OVERWRITTEN each cycle — a
# write inside the markers is clobbered. Content OUTSIDE the markers is a
# legitimate hand-edit surface, so we block ONLY writes that touch the managed
# block, not USER.md wholesale.
USER_MANAGED_BEGIN = "<!-- BEGIN:dinomem-user-map -->"
USER_MANAGED_END = "<!-- END:dinomem-user-map -->"
FORBIDDEN_WHOLESALE = {"MEMORY.md"}  # never a valid target (defense in depth vs ALLOWED_FILES)

def _forbidden_target_error(filename, content=""):
    """Return an error dict if this write hits a forbidden target, else None.
    Enforces route.py's safety floor at the write leaf. Fail-closed on the
    managed-block check (if markers can't be reasoned about, block).
    """
    base = str(filename)
    if base in FORBIDDEN_WHOLESALE:
        return {"ok": False, "error": (
            f"{base} is a FORBIDDEN write target: it is auto-generated by "
            f"extract_memory.py from memory/*.md and overwritten each cycle. "
            f"Write the SOURCE note under memory/ (a _pin_/_note_) instead.")}
    if base == "USER.md" and (USER_MANAGED_BEGIN in content or USER_MANAGED_END in content):
        return {"ok": False, "error": (
            "Refusing to write USER.md's MANAGED BLOCK (dinomem-user-map markers): "
            "it is compiled by compile_user.py from memory/peers/ reps + owner _pin_s "
            "and clobbered next cycle. Write the SOURCE (a memory/ _pin_ or a peer rep) "
            "for biographical facts, or AGENTS.md for a behavioral rule. Content OUTSIDE "
